<script setup>
import { categoryUpdateApi } from "@/js/api/pool.js";
...
const addNewAnswer = () => {
  itemCreateApi(formData.value.id, defaultAnswer(), "category")
    .then((res) => {
...
const onItemPaste = (data) => {
  itemCreateApi(formData.value.id, data, "category")
    .then((res) => {
      formData.value.items.push(res);
    })
    .catch(errorAlert);
...
const onItemRemoved = (index) => {
  ...
  itemDeleteApi(formData.value.id, index, "category").catch(errorAlert);
};
</script>
  ...
  <div>
    <div class="mt-3">
      <TestItem
        ...
        :test-id="formData.id"
        auto-check
        :update-api="(categoryId, i, data) => itemUpdateApi(categoryId, i, data, 'category')"
