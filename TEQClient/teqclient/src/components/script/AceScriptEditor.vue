...
const script = defineModel("script", {default: languages[0].script});
...
const run = ()=>{
  ...
  props.onRun(language.value, script.value ?? "").then(res=>{
    ...
    <div class="card-header">
      <div class="input-group mb-3">
        ...
        <select class="form-select mb-0" id="inputGroupSelect01" v-model="language">
          <option v-for="lang in languages" :key="lang" :value="lang" selected>{{lang.name}}</option>
        </select>
        <button class="btn btn-outline-secondary my-0" type="button" @click="run">{{$t('run')}}</button>
      </div>
    </div>
    <div class="card-body">
      <div id="editor"></div>
