...
const removeMember = (member) => {
    if (member.id) {
        memberDeleteApi(member.id)
            .then(() => removeFromList(member))
            .catch(errorAlert);
    } else {
                ...
                <div class="email-item" v-for="member in members" :key="member">
                    <div class="input-group">
                        <span class="input-group-text"
                            ><i
                                class="fa fa-xmark btn-hover text-danger"
                                @click="() => removeMember(member)"
                            ></i>
                        </span>
                        <p :class="`form-control mb-0 ${!member.id ? 'text-danger' : ''}`">
                            <i class="fa-solid fa-user"></i> {{ member.user.email }}
                        </p>
                    </div>
                </div>
            </div>
